...
/**
 * poolitzer watch — the live feed in a terminal.
 *
...
function frame(): Seg[][] {
  ...
  const status = ` poolitzer watch | Robinhood Chain 4663 | block ${head.toLocaleString("en-US")} | model ${MODEL_ID} . ${trained} | ${launchesPerMinute()} launches/min | ${wsClient ? "ws *" : "poll"}` +
